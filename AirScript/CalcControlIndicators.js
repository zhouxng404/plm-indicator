...
function calcThreeDaysCompleteRate() {
    ...
    if (all && all.length > 0) {
        ...
        months.forEach(function (month) {
            ...
            month_finishFilter.criteria.push({field: '结案天数', op: 'GreaterEqu', values: ['0']});
            month_finishFilter.criteria.push({field: '结案天数', op: 'LessEqu', values: ['3']});
...
function calcInvalidBugRate() {
    ...
    if (all && all.length > 0) {
        ...
        months.forEach(function (month) {
            ...
            month_invalidFilter.criteria.push({
                field: '类型',
                op: 'Intersected',
                values: ['其他(异常/支援等)', '新版本已修复bug']
            });
            month_invalidFilter.criteria.push({
                field: sortField,
